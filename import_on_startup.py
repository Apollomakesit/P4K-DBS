...
async def should_import_csv(db: Database) -> bool:
    ...
    try:
        ...
        if total_players < 1000:
            ...
            logger.info(
                f"📊 Database already has {total_players:,} players - skipping import"
            )
...
async def import_csv_profiles_wrapper(csv_file_path: str = "player_profiles.csv"):
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "import_csv", csv_file_path.replace(".csv", ".py")
        )
...
async def auto_import_on_startup():
    ...
    try:
        ...
        if not csv_file:
            ...
            logger.warning(
                "⚠️ Database will start with current data - use /scan to add more"
            )
            return
        ...
        # Create flag file to mark import as complete
        import_flag = (
            "/data/.csv_imported" if os.path.exists("/data") else ".csv_imported"
        )
        with open(import_flag, "w") as f:
            f.write(f"Import completed at {datetime.now()}\n")
        logger.info(f"✅ Created import flag file: {import_flag}")
