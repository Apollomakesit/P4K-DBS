...
class CooldownTracker:
    ...
    def is_on_cooldown(self, command_name: str, user_id: int, cooldown_seconds: int) -> tuple[bool, int]:
        """
...
def cooldown(seconds: int = 60, admin_bypass: bool = True):
    ...
    def decorator(func):
        ...
        async def wrapper(interaction: discord.Interaction, *args, **kwargs):
            ...
            # Admin bypass
            if admin_bypass and Config.ADMIN_USER_IDS and interaction.user.id in Config.ADMIN_USER_IDS:
...
def create_paginated_embeds(
    ...
    for page in range(total_pages):
        ...
        embed.set_footer(text=f"Showing {start_idx + 1}-{end_idx} of {len(items)} results")
