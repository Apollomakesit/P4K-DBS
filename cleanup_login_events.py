...
def analyze_login_events(conn):
    ...
    logger.info(f"Ratio: {logout_count/login_count:.1f}x more logouts" if login_count > 0 else "No logins")
...
def identify_duplicates(conn, dry_run=True):
    ...
    logger.info(f"Found {len(logout_ids_to_delete):,} duplicate LOGOUT events to delete")
...
def recalculate_session_durations(conn):
    ...
    logger.info(f"  Average session: {stats[2]:.2f} hours" if stats[2] else "  No durations calculated")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Cleanup duplicate login/logout events")
    ...
    try:
        ...
        if args.dry_run or not args.confirm:
            ...
            expected_logouts = logout_count - sum(1 for _ in ids_to_delete if True)  # Approximate
