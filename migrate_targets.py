...
def extract_target_from_text(text: str) -> tuple:
    ...
    match = re.search(r"(?:i-?a|a)\s+dat\s+lui\s+([^(]+)\((\d+)\)", text, re.IGNORECASE)
    if match:
    ...
    match = re.search(r"lui\s+\((\d+)\)", text, re.IGNORECASE)
    if match:
...
def migrate_targets(
    ...
    logger.info(
        f"With target_player_id: {stats['has_target_id']:,} ({stats['has_target_id']/stats['total']*100:.1f}%)"
    )
    logger.info(
        f"Missing target_player_id: {stats['missing_target_id']:,} ({stats['missing_target_id']/stats['total']*100:.1f}%)"
    )
    ...
    while total_processed < total_to_process:
        ...
        logger.info(
            f"📦 Batch {batch_num}: Processing {len(actions)} actions (total: {total_processed:,}/{total_to_process:,})"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate target player info into actions table"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
