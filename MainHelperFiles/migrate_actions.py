...
def migrate_actions(dry_run: bool = False, batch_size: int = 1000):
    ...
    while offset < total_count:
        ...
        logger.info(
            f"📦 Processing batch {batch_num} ({len(actions)} actions, offset {offset:,}/{total_count:,})"
        )
        ...
        if batch_num % 5 == 0:
            ...
            logger.info(
                f"   📈 Progress: {pct:.1f}% ({stats['processed']:,}/{total_count:,})"
            )
    ...
    print("=" * 60)
    print(
        f"{'Mode:':<25} {'DRY RUN (no changes made)' if dry_run else 'LIVE (changes applied)'}"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate old actions to new categorized types"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--batch-size", type=int, default=1000, help="Actions per batch (default: 1000)"
    )
